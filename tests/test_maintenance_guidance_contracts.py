...
class MaintenanceGuidanceContractTests(unittest.TestCase):
    def test_organize_docs_migration_reference_resolves(self) -> None:
        skill_path = REPO_ROOT / "skills/organize-docs/SKILL.md"
        migration_links = [
            link
            for link in markdown_links(skill_path.read_text(encoding="utf-8"))
            if Path(link).name == "legacy-claude-migration.md"
        ]
        self.assertEqual(1, len(migration_links))
        self.assertTrue((skill_path.parent / migration_links[0]).is_file())
