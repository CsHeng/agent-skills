...
description: "Implement an approved bounded change through mutation, verification, one bounded review, accepted repair, and an evidence-backed outcome."
---

# Implement Change

Complete an approved implementation plan while preserving its scope, authority, and acceptance evidence.

## Use This Skill When

- an approved plan authorizes repository mutation
- implementation, verification, review adjudication, and bounded repair need one semantic owner
...
- Confirm the approved objective, task order, allowed surfaces, declared verification, recovery policy, and known user changes.
...
7. Invoke `review-change` exactly once over the converged bounded implementation diff. Supply the objective, scope, changed files, declared oracles, and current verification evidence.
8. Adjudicate every material review candidate. Accept only findings causally tied to the current change and fixable inside the approved scope.
9. If accepted findings exist, apply at most one focused same-slice repair and rerun the affected and declared verification without starting another review or an unbounded repair loop.

This review is part of a formal `implement-change` invocation. Ordinary informal or non-formal edits do not acquire review merely because this Skill exists.
...
- `pass`: implementation and required verification pass; no accepted finding remains
...
Return the outcome with changed files, verification evidence, review verdict, adjudication, repair evidence when applicable, and remaining uncertainty. Do not claim a write, install, deploy, commit, or push that was not actually performed.
