...
def _has_forbidden_path_character(value: str) -> bool:
    ...
    ) or any(character in GLOB_CHARACTERS for character in value)
...
def _canonical_existing_regular_file(repo_root: Path, declared_ref: str) -> Path:
    if not declared_ref or not os.path.isabs(declared_ref):
        raise ExternalTouchError(
            "external_touch_invalid_path", "external ref must be absolute"
        )
...
def stage_payload(
    *, run_dir: Path, intent_id: str, source_file: Path
) -> dict[str, object]:
...
def declare_intent(
    *,
    ...
    chain = _intent_chain(
        baseline_ref, intents, ref, require_applied=True, require_cleanup=True
    )
    ...
    parent = (
        dict(cast(Mapping[str, object], chain[-1]["after"]))
        if chain
        else dict(baseline_ref)
    )
...
def stage_declared_payload(
    *, intent: Mapping[str, object], source_file: Path
) -> dict[str, object]:
...
def _intent_chain(
    ...
    *,
    ...
    for sequence, intent in enumerate(selected, start=1):
        ...
        if not _evidence_equal(root, baseline_ref) or not _evidence_equal(
...
def _broker_candidate_path(
    *, run_id: str, task_id: str, ref: str, intent_id: str
) -> Path:
    opaque_id = hashlib.sha256(
        f"{run_id}\0{task_id}\0{ref}\0{intent_id}".encode()
    ).hexdigest()[:24]
...
def prepare_intent(
    *,
    ...
    if any(intent.get("intent_id") == intent_id for intent in intents):
        raise ExternalTouchError(
            "external_touch_duplicate_intent", "intent ID must be unique"
        )
    parent = (
        dict(cast(Mapping[str, object], chain[-1]["after"]))
        if chain
...
def _candidate_matches_target(
    current: Mapping[str, object], intent: Mapping[str, object]
) -> bool:
    candidate = cast(Mapping[str, object], intent.get("candidate", {}))
    parent = cast(Mapping[str, object], intent.get("parent", {}))
...
def durable_replace_file(
    *, staged_file: Path, destination_file: Path
) -> dict[str, object]:
...
def apply_intent(*, repo_root: Path, intent: Mapping[str, object]) -> dict[str, object]:
    ...
    _copy_broker_candidate(
        staged_path=staged_path, candidate_path=candidate_path, parent=parent
    )
...
def cleanup_intent(
    *, intent: Mapping[str, object], allow_prepared: bool = False
    ...
    try:
        ...
        _require_token(run_id, "run_id")
        _require_token(task_id, "task_id")
        canonical_run_dir = _validate_run_dir(
            Path(cast(str, candidate.get("run_dir", "")))
        )
    except ExternalTouchError as exc:
        raise ExternalTouchError(
            "external_touch_cleanup_ambiguous",
...
def apply_and_cleanup_intent(
    *, repo_root: Path, intent: Mapping[str, object]
) -> dict[str, object]:
...
def validate_evidence_state(
    *,
    ...
    if (
        ...
        raise ExternalTouchError(
            "external_touch_evidence_invalid", "baseline schema is not exact"
        )
    if (
    ...
    for baseline_ref in refs:
        ...
        for intent in chain:
            ...
            if metadata != {
                ...
                for field in ("file_type", "mode", "uid", "gid")
            }:
                raise ExternalTouchError(
                    "external_touch_evidence_invalid",
            ...
            if state == "applied":
                after = cast(Mapping[str, object], intent.get("after", {}))
                if set(after) != file_keys or not _candidate_matches_target(
                    after, intent
                ):
                    raise ExternalTouchError(
                        "external_touch_evidence_invalid",
    ...
    *,
    ...
    for baseline_ref in refs:
        ...
        for intent in chain:
            _validate_cleanup_absent(intent)
        expected = (
            cast(Mapping[str, object], chain[-1]["after"]) if chain else baseline_ref
        )
        current = capture_file_evidence(repo_root, ref)
