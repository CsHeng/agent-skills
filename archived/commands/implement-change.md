...
Resolve and validate the approved plan with `bash "${CLAUDE_PLUGIN_ROOT}/skills/implement-change/scripts/harness/execute-runner.sh"`. Require `approval_status: approved`, materialize the task catalog and task ledger, perform the one-time worktree preflight, and execute inside `allowed_touch_set = impl_file_refs + test_file_refs`.
