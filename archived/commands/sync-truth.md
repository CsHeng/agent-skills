...
```bash
RUNNER=""
if [[ -n "${CLAUDE_PLUGIN_ROOT:-}" ]] && [[ -f "${CLAUDE_PLUGIN_ROOT}/skills/sync-truth/scripts/harness/truth-sync-runner.sh" ]]; then
  RUNNER="$(realpath "${CLAUDE_PLUGIN_ROOT}/skills/sync-truth/scripts/harness/truth-sync-runner.sh")"
fi
if [[ -z "${RUNNER:-}" ]]; then
  RUNNER="$(find ~/.claude/plugins -path '*/skills/sync-truth/scripts/harness/truth-sync-runner.sh' -print -quit 2>/dev/null)"
