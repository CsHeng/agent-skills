...
The portable install unit is one generated directory under `skills/<public-id>`. Each directory contains its own `SKILL.md`, projected provider metadata, and referenced resources. It does not resolve repository siblings or require executable workflow support. Skills with `distributed = false` remain public IDs with authored sources, but flatten omits them from this payload.

The root-flat `skills/` tree is generated from distributed authored sources and checked for exact parity and reference closure. A local checkout with child symlinks is the recommended update path. Plugin manifests are optional packaging metadata and do not change Skill semantics.
