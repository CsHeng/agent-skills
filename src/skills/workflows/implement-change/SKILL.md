...
The approved objective, non-goals, dependencies, authority, protected contracts, and acceptance conditions remain the baseline throughout repair. Updating progress or adding execution evidence does not authorize rewriting these requirements to match the current implementation. Distinguish a listed capability, permission to run its gate, and evidence required for this delivery: none implies the others. Before treating an unavailable gate as a completion blocker, identify the approved outcome or changed behavior that needs it. Prepare obtainable prerequisites within existing authority; retain genuine required capability gaps, and never relabel required positive evidence as optional or satisfied by a negative preflight.
