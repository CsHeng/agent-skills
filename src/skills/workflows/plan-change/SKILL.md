...
11. Close every implementation-oriented plan with an explicit implementation-approval summary for the user. State the exact scope and actions approval would cover, authority already covered, unresolved `manual_checkpoint` items, actions intentionally excluded, and the limited conditions that would require another pause. Keep that summary to currently effective goals, discretion, and real pause conditions; cite historical exemptions shortly instead of copying old gates. Do not hide a future approval request in task prose or ask again for ordinary in-scope execution choices after the stated scope is approved.
