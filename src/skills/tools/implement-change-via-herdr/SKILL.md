...
This is an explicit lower-plane adapter. It consumes the immutable controller-binding envelope created by `implement-change` for the Herdr backend — the `schema_version: 1` wire shape emitted only with explicit `--backend herdr` selection. Flag-absent selection and codex-native `schema_version: 2` envelopes are not for this adapter. It never selects tasks, changes the ledger, converges work, reviews, repairs, or derives a lifecycle tail.
